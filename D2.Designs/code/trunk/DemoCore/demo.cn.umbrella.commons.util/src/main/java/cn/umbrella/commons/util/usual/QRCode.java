		...
		for (int y = 0; y < matrix.getHeight(); y++) {
			for (int x = 0; x < matrix.getWidth(); x++) {
				...
				if(x>0&&x<170&&y>0&&y<170){
					Color color= new Color(11,31,242);
					int colorInt=color.getRGB();
					pixels[y * width + x] = matrix.get(x, y) ? colorInt: 16777215;
				}
				
				else if(x>0&&x<170&&y>342&&y<512){
					Color color= new Color(231,144,56);
					int colorInt=color.getRGB();
					pixels[y * width + x] = matrix.get(x, y) ? colorInt: 16777215;
				}
				
				else if(x>342&&x<512&&y>0&&y<170){
					Color color= new Color(231,144,56);
					int colorInt=color.getRGB();
					pixels[y * width + x] = matrix.get(x, y) ? colorInt: 16777215;
				}
				...
				// 在图片四周形成边框
						...
						|| (x > halfW - IMAGE_HALF_WIDTH - FRAME_WIDTH
								...
								+ IMAGE_HALF_WIDTH + FRAME_WIDTH)) {
					//图片边框颜色
					pixels[y * width + x] = 0x99cc32;
				} else {
					
					//二维码颜色   y+1  ===》 从  1 到 height
					//num1   50 ---> 13
...
//					System.out.println("   num1: " + num1 + "   num2: " + num2 + "   num3: " + num3);
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		QRCode.encode("http://k.go189.cn/ztmsxy_yfb/pageredirect/redirectIndex", 512, 512, "E:/100.png", "E:/101.png");
	}
}
